...
import com.qian.utils.AnimUtils;
...
public class VideoController extends FrameLayout
    ...
    private Runnable progressCallback = new Runnable() {
        ...
        public void run() {
            if (isVideoReadyToBePlayed && isVideoSizeKnown) {
                int progress = KTimeUtils.millsToSec((int) mediaPlayer.getCurrentPosition());
                currentTime.setText(KTimeUtils.secToTime(progress));
    ...
    public void surfaceCreated() {
        ...
        endTime.setText(KTimeUtils.secToTime((int) item.data.duration));
        mVideoWidth = ScreenUtils.getScreenWidth();
        mVideoHeight = ScreenUtils.getScreenHeight();
    ...
    public void calculateVideoWidthAndHeight() {
        LayoutParams lp = (LayoutParams) centerLayout.getLayoutParams();
                ...
                if (Math.abs(moveProgress) > 0) {
                    ...
                    progressTime.setText(
                            getResources().getString(
                                    ...
                                    KTimeUtils.secToTime(currentProgress + moveProgress),
                                    KTimeUtils.secToTime(maxProgress)));
                }
                break;
